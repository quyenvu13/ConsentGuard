...
let parameterReadsAvailable = MOCK;
...
async function loadUserState(user){if(!validAddress(user)){consentCache=null;actionCountCache=null;return;}if(!MOCK){consentCache=null;actionCountCache=null;parameterReadsAvailable=false;return;}const c=await readClient();consentCache=asBool(await c.readContract({address:CONTRACT,functionName:'has_valid_consent',args:[user]}));actionCountCache=Number(parseResult(await c.readContract({address:CONTRACT,functionName:'get_action_count',args:[user]})));}
